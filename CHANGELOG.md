...
- **Verified live via computer-use:** drew a Rect (visible handles + X) → clicked elsewhere → handles cleanly disappeared, no shape created (single click below the drag-distance threshold).
- **Bumped V1.0041 → V1.0042** per Critical Rule #12.

### Fixed — V1.0041: placed-element selection chrome clears on background click + ⌘S saves in place for opened files (2026-04-30)
- **Selection handles around a placed image/text/shape now disappear when you click elsewhere on the page.** Pre-V1.0041 the resize handles, crop handle, X delete badge, and font-size +/- buttons stayed visible until you pressed Escape or restarted. New scroller-level `onPointerDown` in [src/renderer/components/Viewer/Viewer.tsx](src/renderer/components/Viewer/Viewer.tsx) calls a new `clearAllPendingSelections()` action whenever the click target isn't inside a `[data-pending-element]` ancestor. Pending layers (image, text, shape) all call `e.stopPropagation()` in their own pointer-down so the scroller handler fires only for true background clicks; two shape variants (rect, ellipse) were missing the stopPropagation and got it added.
- **⌘S now saves in place on a file you just opened.** Pre-V1.0041 the FIRST ⌘S after opening a PDF routed to a Save-As dialog because `addTab` defaulted `saveInPlace: false`. Most users (and every other Mac app) expect ⌘S to overwrite the file they're editing — the dialog read as "Save As" and got dismissed mid-edit. Fix in [stores/document.ts](src/renderer/stores/document.ts): `saveInPlace: init.saveInPlace ?? !!init.path`. Tabs with a real path save in place by default. Decrypted-from-encrypted PDFs already drop `path` to `null` (App.tsx wasDecrypted branch), so they still force Save-As — Critical Rule #6 ("never auto-save over the user's original" for encrypted-source tabs) still holds. Virtual tabs (combine / image-import) also keep Save-As because they're path-less.
- **Bumped V1.0040 → V1.0041** per Critical Rule #12.
...
- **Two-layer root cause.** First attempt only patched the inner layer and the bug still reproduced; had to fix the parent too.
  1. **Inner:** [src/renderer/components/Viewer/AcroFormLayer.tsx](src/renderer/components/Viewer/AcroFormLayer.tsx) used `key={\`${widget.name}-${index}-${activeTab.version}\`}` for each `<FieldWidget>`. Stabilised to `${widget.name}-${index}`.
  2. **Outer (the actual fix):** [src/renderer/components/Viewer/Viewer.tsx](src/renderer/components/Viewer/Viewer.tsx) keyed `<PageCanvas>` on `${activeTab.id}-${activeTab.version}-${p}`. `version` bumps on every `applyEdit`, remounting the entire page subtree — canvas, text layer, AcroFormLayer, and all `<input>` elements inside it. The comment at the call site claimed the version-in-key was needed to "force a fresh canvas after edits," but PageCanvas's render `useEffect` already depends on `pdf` and re-builds the canvas + text layer on every `pdf` prop change. The version-in-key was unnecessary paranoia. Stabilised to `${activeTab.id}-${p}`.
- **Why it manifested as Tab failure:** When user blurred Postal Code (Tab), the commit fired `setFormFields → applyEdit → version++`. Tab moved browser focus to Contact's `<input>`. React then unmounted+remounted the entire page subtree because of the version-in-key. The Contact `<input>` element the browser had just focused was destroyed; the next keystrokes landed on whatever default focus target took its place (nothing useful). Stable key → DOM identity preserved → Tab focus survives the async commit.
- **Verified end-to-end via computer-use:** Postal Code "M5V 3A8" → Tab → Contact "Adam Hayat" → Tab → Email "me@adamhayat.ca". All three fields retained their values, baked appearances rendered correctly via pdf.js's appearance pipeline.
- **Bumped V1.0038 → V1.0039** per Critical Rule #12.
...
- **Duplicate guard added in main.** If both the notification and fallback URL arrive for the same action, `queueOrHandleWeavePdfUrl` ignores the duplicate for 5 seconds so in-place verbs don't run twice.
- **Bumped V1.0033 → V1.0034** per Critical Rule #12.
- **Tests:** `npm run typecheck`, `node scripts/build-url-listener.mjs`, `node scripts/build-finder-sync.mjs`, and `npm run package` all pass. Installed to `/Applications/WeavePDF.app`, killed stale `WeavePDFFinderSync` extension processes so pkd reloads the new bundle, then verified both the `NSWorkspace.open(..., activates: false)` fallback and the acknowledged notification path convert a Desktop PNG into a PDF.

### Fixed — V1.0033: Show Desktop stays active for warm Finder actions (2026-04-29)
- **Right-click background actions no longer collapse macOS Show Desktop when WeavePDF is already running.** `NSWorkspace.open(..., activates: false)` was enough to avoid ordinary focus steal in V1.0032, but LaunchServices can still disturb Show Desktop while delivering a custom URL to an already-running app. Finder Sync now posts a local distributed notification for background-only verbs (Quick Compress / Convert / Extract first page / Rotate clockwise / Rotate counterclockwise) when the parent app is already alive, bypassing LaunchServices entirely.
- **Added a tiny native notification bridge** ([resources/helpers/url-listener.swift](resources/helpers/url-listener.swift), [src/main/main.ts](src/main/main.ts)). The helper listens for `ca.adamhayat.weavepdf.finder-action` notifications and prints the `weavepdf://` URL to stdout; Electron main feeds that into the existing `handleWeavePdfUrl` pipeline, so all validation, logging, and PDF operations stay in one place.
- **Cold-start fallback preserved.** If WeavePDF is not running, Finder Sync still uses the existing `NSWorkspace.open` URL dispatch so the action can run. `Combine into PDF` also keeps using LaunchServices because it intentionally opens the merged result in WeavePDF.
- **Build hook updated** so `npm run package` compiles and embeds `resources/helpers/url-listener-bin` alongside the Finder Sync extension.
- **Bumped V1.0032 → V1.0033** per Critical Rule #12.
- **Tests:** `npm run typecheck` clean against `weavepdf@1.0.33`. `node scripts/build-url-listener.mjs`, `node scripts/build-finder-sync.mjs`, and `npm run package` all pass. Installed to `/Applications/WeavePDF.app`; warm installed-app notification smoke confirmed the bridge receives a posted `weavepdf://convert?...` URL and the existing handler writes the converted PDF.
