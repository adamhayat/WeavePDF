...
> **Then `BRAND.md`** for the canonical brand identity (name, voice, palette, type, icon) — `BRAND.md` takes precedence over this file for voice and visual decisions. This file still wins on engineering rules.
> **Update `HANDOFF.md` and `CHANGELOG.md` after every change — same turn, not later.** See Critical Rule #0.

Local Mac-native alternative to Adobe Acrobat. No cloud, no account, no subscription. Target user: Adam (personal use on macOS). Current version: **V1.0002** — see `HANDOFF.md` for the full state. Display version comes from `package.json`'s semver patch field, padded to 4 digits (semver `1.0.2` → display `V1.0002`). Bump per Critical Rule #12.

The project shipped a full rename from "Acrofox" to **WeavePDF** in V1.0002 (see `HANDOFF.md` 2026-04-28 session log entry). Bundle ID is now `ca.adamhayat.weavepdf`, the installed app is `/Applications/WeavePDF.app`, the Finder Quick Actions are "Compress / Convert to PDF / Extract first page / Combine into PDF / Rotate 90° **with WeavePDF**", and the accent color shifted from electric violet to Loom Indigo. The new app icon is a flat indigo squircle with a page glyph and two crossing threads forming a `W`. All historical references in this file and in HANDOFF/CHANGELOG that still mention "Acrofox" are intentional historical context — do not "fix" them.
...
npm run dev           # Vite renderer + Electron main, HMR enabled
                      # If running under Claude/automation, wrap with:
...
npm test              # Runs the packaged Playwright suite (currently 43 specs; see HANDOFF.md)

# Publish a release to GitHub
npm run release       # Builds DMG via `npm run make`, extracts the V1.<patch>
                      # CHANGELOG block as release notes, creates a GitHub
                      # Release at github.com/adamhayat/WeavePDF, uploads
                      # the .dmg + .zip as assets. Friends running an older
                      # WeavePDF see the in-app update prompt on next launch.
                      # Requires: gh CLI authenticated, clean working tree,
                      # tag vX.Y.Z not already on origin.
                      # Override dirty check: WEAVEPDF_DIRTY_OK=1 npm run release
                      # Draft a release for review: npm run release -- --draft
...
### Pushing a release to everyone

When the user says **"push the new release to everyone"**, **"ship this to everyone"**, **"release this"**, or any close variant, run this end-to-end checklist *without further prompting* — the trigger phrase IS the explicit permission to commit + push + publish. Do NOT ask "are you sure" between steps; the user is asking for the whole flow. Stop only on actual failure.

1. **Confirm version bumped.** Per Critical Rule #12 the patch in `package.json` should already reflect the work in this session. If not, bump it now.
2. **Confirm docs synced.** `HANDOFF.md` Status line + session log entry, `CHANGELOG.md` `[Unreleased]` block — both reference the new version. Per Critical Rule #0, these should already be in place. If not, write them now.
3. **Typecheck:** `npm run typecheck`. Fail → stop, surface the error, don't push a broken release.
4. **Repackage + reinstall locally:** the `npm run package` + `pkill /Applications/WeavePDF` + `cp -R out/.../WeavePDF.app /Applications/` snippet. Confirms the local install matches what's about to ship.
5. **Commit + push** to `github.com/adamhayat/WeavePDF`:
   ```bash
   git add -A
   git commit -m "WeavePDF V1.0<patch4> — <one-line summary>"
   git push origin main
   ```
   Use the same one-line summary used in `HANDOFF.md`'s session log heading. Use the [hereDoc commit-message form](#) from the global rules so multi-line summaries format correctly.
6. **Publish the release:** `npm run release`. The script handles DMG build, CHANGELOG block extraction, `gh release create`, asset upload.
7. **Report the release URL** back to the user (`https://github.com/adamhayat/WeavePDF/releases/tag/v1.0.<patch>`).

Friends running an older WeavePDF will see the in-app **"WeavePDF V1.0… is available"** dialog the next time they launch the app (auto-poll runs 5 seconds after window load, every launch). They click **Download** → release page opens in browser → drag new DMG to /Applications. *Manual install is the current Tier 1 limit — Tier 2 silent auto-install needs Apple Developer ID ($99/yr).*

Side note: if the user says **"check for updates"** or asks whether the auto-check works, the answer is yes — `app.whenReady` schedules `checkForUpdatesAndNotify({ silentIfUpToDate: true })` 5 seconds after window load on every cold launch. The Help → Check for Updates… menu item triggers the same logic with `silentIfUpToDate: false` so manual checks always show a dialog.
...
12. **Always bump the version on every shipped change.** Single source of truth is `package.json`'s `"version"` (semver). Format is `1.0.<patch>`; the user-facing display is `V1.0<patch4>` (e.g. semver `1.0.1` → `V1.0001`, `1.0.42` → `V1.0042`). Increment the patch by 1 on every code-changing turn — bug fix, feature, doc-only refactor that ships with code, anything that goes into the next packaged build. Surfaces that show the version: macOS About panel (set in [main.ts](src/main/main.ts) via `app.setAboutPanelOptions`) and the footer of the `⌘/` Keyboard Shortcuts panel ([ShortcutHelpModal.tsx](src/renderer/components/ShortcutHelpModal/ShortcutHelpModal.tsx)). Both compute the display from `package.json` — touch only `package.json`. Note the new version in the same turn's `HANDOFF.md` "Current state" line and the `CHANGELOG.md` `[Unreleased]` section header.
...
`npm test` runs the full packaged Playwright suite (currently 43 specs across 4 files). You can target a single spec with `npx playwright test edit.spec.ts`. Traces + screenshots land in `test-results/` on failure.
